...
int lca(int x, int y){
    if(depth[y]<depth[x]) swap(y, x); // y가 더 깊음을 보장
    for(int k=MAX_K-1; k>=0; k--){ // x, y의 depth를 맞춤
        if(depth[y]-depth[x] >= 1<<k) y = dp[y][k];
    }
    if(x==y) return x; // x가 LCA(최소 공통 조상)인 경우
    for(int k=MAX_K-1; k>=0; k--){ // LCA를 찾음
        if(dp[x][k]!=dp[y][k]){
            x = dp[x][k];
            y = dp[y][k];
        }
...
int main(){
    ...
    cin >> M;
    for(int i=0; i<M; i++){ // 한동이가 방문해야 할 모든 도시를 방문 할 수 있는 최소 시간을 구함
        cin >> now;
