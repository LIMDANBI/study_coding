...
int dc[] = { 0, 1, -1, 0, 0 };
int dh[5][2][3] = { // dh[1][0][0] 오른쪽(1, RIGHT)을 바라볼 때 r 좌표 0번째
	{{ 0, 0, 0 }, { 0, 0, 0 }},
	{{ -1, 0, 1 }, { 1, 1, 1 }}, // 오른쪽
	{{ -1, 0, 1 }, { -1, -1, -1 }}, // 왼쪽
	{{ -1, -1, -1 }, { -1, 0, 1}}, // 위
...
void input() {
	...
	for (int i = 1; i <= R; i++) { //  집 상태 정보
		for (int j = 1; j <= C; j++) {
			cin >> map[i][j];
			if (RIGHT <= map[i][j] && map[i][j] <= DOWN) heater.push_back({ i, j, map[i][j] });
			if (map[i][j] == CHECK) checkPos.push_back({ i,j });
		}
	}
	cin >> W;
	int x, y, t;
	for (int i = 0; i < W; i++) { // 벽 정보 -  t = 0, (x, y)와 (x-1, y) 사이 벽 / t = 1, (x, y)와 (x, y+1) 사이 벽
		cin >> x >> y >> t;
		if (t == 0) {
