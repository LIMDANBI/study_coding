...
struct BOMB
{
	int size, red, row, col;
	bool operator < (const BOMB &b) const {
		if (size == b.size) {
			if (red == b.red) {
				if (row == b.row) return col > b.col; // 기준 블록의 열이 가장 작은 것 (4)
				return row < b.row; // 기준 블록의 행이 가장 큰 것 (3)
			} return red > b.red; // 빨간 폭탄이 가장 적은 것 (2)
...
void turn_on_red() {
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			if (map[i][j] == RED) visited[i][j] = false;
		}
	}
}
...
pair<int, int> bfs(int i, int j, int num) {
	...
	while (!q.empty()) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (map[ny][nx] != RED && map[ny][nx] != num) continue;
			if (map[ny][nx] == RED) rbow++;
			visited[ny][nx] = true;
			q.push({ ny,nx });
			size++;
		}
...
pair<int, int> find_block_group() {
	...
	for (int i = N-1; i >= 0; i--) { // 기준점!!!!!!!!!!! 행 큰 것 부터!!!!!!!!!!!!
		for (int j = 0; j <N; j++) { // 행이 큰 것 여러개라면, 열이 작은 것 !!
			if (visited[i][j] || map[i][j] == RED || map[i][j] == BLACK || map[i][j] == EMPTY) continue;
			pair<int, int> res = bfs(i, j, map[i][j]);
			turn_on_red(); // 빨간 폭탄은 중복으로 가질 수 있음!
			if (res.first >= 2) pq.push({ res.first, res.second, i, j });
		}
	}

	if (pq.empty()) {  // 더 이상 폭탄 그룹이 없는 경우
		isEnd = true;
		return { -1, -1 };
	}
	...
	return { pq.top().row, pq.top().col };  // 우선순위가 높은 폭탄 그룹의 기준점
}
...
void remove_block_group(int y, int x, int num) {
	...
	while (!q.empty()) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (map[ny][nx] == num || map[ny][nx] == RED) {
				visited[ny][nx] = true;
				map[ny][nx] = EMPTY;
				q.push({ ny,nx });
			}
